...
def send_email(mail, app, to, subject, body):
    try:
    ...
    def forgot_password():
        try:
            ...
            if user:
                ...
                reset_link = f"{app.config['FRONTEND_URL']}?reset={token}"
...
Best regards,
Fileapp Team"""
                send_email(mail, app, user.email, subject, body)
    ...
    def create_share():
        try:
            ...
            user = User.query.get(user_id)
            share_url = f"{app.config['FRONTEND_URL']}?token={token}"
...
Fileapp Team"""
            ...
            send_email(mail, app, data['recipient_email'], subject, body)
